...
export default function Footer() {
    return (
        <main className="w-full py-8 px-6 bg-primary/20 flex flex-col md:flex-row md:justify-between gap-6 md:gap-0">
            <aside className="flex flex-col gap-3">
                <div className='flex gap-1 text-destructive items-center '>
                    <Vault size={25} className="text-destructive" />
                    <h3>Openvault</h3>
                </div>
                <h6 className="max-w-[300px] text-start">The decentralized saving protocol for the offline world.EMpowering financial freedon withouth boundaries</h6>
                ...
                { 
                    [
                        ...
                        <article key={index} className="flex flex-col gap-3">
                            <h4 className="uppercase text-[0.85rem] md:text-md">{item.header}</h4>
                            <div className="flex flex-col gap-3">
                                {item.children.map((text, counter) => (
                                    <h6 key={counter} className="uppercase hover:underline text-[0.65rem] font-bold">
                                        {text}
                                    </h6>
                                ))}
                            </div>
